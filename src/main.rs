...
fn main() {
    ...
    let mut i = 0;
    while start_time.elapsed().as_millis() < 94 {
    ...
    eprintln!("{} {}", i, start_time.elapsed().as_millis());

    let mut best = population[0];
    for i in 1..population.len() {
        // Clone the objects and use mut references to call fitness()
        let mut best_clone = best.clone();
        let mut current_clone = population[i].clone();
        if my_lib::genetics::pheno::Phenotype::fitness(&mut current_clone)
            > my_lib::genetics::pheno::Phenotype::fitness(&mut best_clone)
        {
            best = population[i];
        }
    }
    ...
    loop {
        ...
        eprintln!("fitness : {} ", my_lib::genetics::pheno::Phenotype::fitness(&mut best));
        let rot = my_lib::entities::genome::get_rotate_on_turn(best.get_genome(), 0);
        starship.add_rotation(rot);
        let thrust = my_lib::entities::genome::get_power_on_turn(best.get_genome(), 0);
        starship.add_power(thrust as i32);
